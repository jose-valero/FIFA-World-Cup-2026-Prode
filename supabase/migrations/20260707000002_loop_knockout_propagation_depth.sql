-- ============================================================
-- Fix: sync_qualified_teams_into_knockout — propagación a
-- múltiples niveles de profundidad en una sola llamada.
--
-- La corrección anterior (20260707000001) quitó el guard roto
-- contra tmp_previous_knockout_state, pero eso no alcanza: la
-- Fase 5/6 sigue siendo un único UPDATE ... FROM sm que lee una
-- sola foto consistente de public.matches al inicio de la
-- sentencia. Para el salto octavos -> cuartos, sm (el partido de
-- octavos) todavía aparece con home_team_id/away_team_id en NULL
-- en esa foto (recién reseteados por la Fase 1), aun cuando esa
-- misma sentencia esté a punto de recalcularlos para otras filas.
-- Postgres nunca deja que una sentencia UPDATE vea sus propios
-- cambios a otras filas dentro de la misma sentencia.
--
-- Profundidad máxima del bracket:
--   R32 (fase 3/4) -> R16 (1) -> QF (2) -> SF (3) -> Final/3ro (4)
--
-- Fix: repetir las Fases 5 y 6 (4 sentencias UPDATE) dentro de un
-- loop de 4 iteraciones. Cada iteración es una sentencia nueva que
-- sí ve los commits de la iteración anterior dentro de la misma
-- transacción, permitiendo que la cascada llegue hasta el final.
-- Recalcular un nivel ya resuelto es un no-op (mismos valores).
-- ============================================================
...
begin
  ...
  -- ── Fase 5/6: propagar ganadores y perdedores de knockout ────────────────
  -- Se repite en un loop porque cada iteración es una sentencia SQL nueva
  -- (ve los commits de la iteración anterior dentro de la misma
  -- transacción). Profundidad máxima del bracket: R16 -> QF -> SF ->
  -- Final/3er puesto = 4 niveles, así que 4 iteraciones garantizan que la
  -- cascada llegue completa en una sola llamada a la función.
  for v_depth in 1..4 loop
    ...
    get diagnostics v_pass_updates = row_count;
    v_winner_home_updates := v_winner_home_updates + v_pass_updates;
    ...
    get diagnostics v_pass_updates = row_count;
    v_winner_away_updates := v_winner_away_updates + v_pass_updates;
    ...
    get diagnostics v_pass_updates = row_count;
    v_loser_home_updates := v_loser_home_updates + v_pass_updates;
    ...
    get diagnostics v_pass_updates = row_count;
    v_loser_away_updates := v_loser_away_updates + v_pass_updates;
