-- ============================================================
-- Fix: sync_qualified_teams_into_knockout no propaga ganadores
-- más allá de un salto (octavos -> cuartos, cuartos -> semis, etc.)
--
-- Problema: Fase 1 resetea a NULL el home_team_id/away_team_id de
-- todo partido cuyo origen sea match_winner/match_loser en CADA
-- corrida (solo preserva best_third_place). Fase 5/6 repuebla esos
-- campos leyendo el partido origen (sm) desde un guard:
--
--   and not (
...
--   )
--
-- Ese guard compara sm (ya reseteado a NULL por la Fase 1 de esta
-- misma corrida, para cualquier partido sourced por match_winner/
-- match_loser) contra ps.prev_* (su valor real previo a la corrida).
-- Para cualquier propagación de profundidad >= 2 (octavos->cuartos,
-- cuartos->semis, semis->final/3er puesto), sm siempre es NULL en
-- ese punto y ps.prev siempre es un uuid real una vez que el
-- partido de origen quedó resuelto, así que el guard nunca pasa:
-- esos partidos jamás reciben sus equipos reales, sin importar
-- cuántas veces se vuelva a correr el sync.
--
-- Octavos se resuelve bien porque su origen (round_of_32) no se
-- resetea en Fase 1 (viene de group_position/best_third_place), así
-- que el guard pasa ahí trivialmente. Pero eso mismo hace que sea
-- inútil: nunca bloquea un caso legítimo, solo bloquea para siempre
-- los casos de profundidad >= 2.
--
-- La reversibilidad real (invalidar resultados de partidos cuyo
-- cruce cambió) ya la cubre la Fase 7 usando la misma tabla
-- tmp_previous_knockout_state, así que este guard es redundante y
-- se elimina de las Fases 5 y 6.
-- ============================================================
...
begin
  ...
  --
  -- NOTA: no se filtra contra tmp_previous_knockout_state acá. sm (el partido
  -- origen) puede ser en sí mismo un partido sourced por match_winner/loser
  -- que la Fase 1 acaba de resetear a NULL y esta misma Fase 5 recién está
  -- recalculando (p.ej. octavos, cuando se propaga a cuartos). Comparar sm
  -- contra su valor previo a la corrida bloqueaba para siempre cualquier
  -- propagación de profundidad >= 2. La reversibilidad de resultados ante
  -- cambios de cruce la cubre la Fase 7 más abajo.
  update public.matches m
  set
    home_team_id   = case
