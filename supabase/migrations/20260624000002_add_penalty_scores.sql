-- ============================================================
-- Fase 2: soporte para marcadores de penales en knockout
--
-- Cambios:
--   1. Agrega penalty_home_score y penalty_away_score a matches.
--      - Ambas nullable (null = partido sin penales / no cargado aún).
--      - Constraint: ambas null o ambas not null.
--      - Constraint: si existen, deben ser distintas (siempre hay ganador).
--
--   2. Actualiza sync_qualified_teams_into_knockout():
--      - Fases 5 y 6: la condición de propagación acepta empate + penales.
--      - El CASE winner/loser usa penalty_score como desempate final.
--      - Fase 7: también limpia penalty scores al invalidar partidos.
--
-- ESPN no provee penalty scores en el scoreboard.
-- Los scores de penales se cargan manualmente desde admin/resultados.
-- ============================================================

ALTER TABLE public.matches
  ADD COLUMN IF NOT EXISTS penalty_home_score smallint,
  ADD COLUMN IF NOT EXISTS penalty_away_score  smallint;

ALTER TABLE public.matches
  ADD CONSTRAINT chk_penalty_home_score_non_negative
    CHECK (penalty_home_score IS NULL OR penalty_home_score >= 0),
  ADD CONSTRAINT chk_penalty_away_score_non_negative
    CHECK (penalty_away_score IS NULL OR penalty_away_score >= 0),
  ADD CONSTRAINT chk_penalty_scores_together
    CHECK ((penalty_home_score IS NULL) = (penalty_away_score IS NULL)),
  ADD CONSTRAINT chk_penalty_scores_decisive
    CHECK (penalty_home_score IS NULL OR penalty_home_score <> penalty_away_score);
